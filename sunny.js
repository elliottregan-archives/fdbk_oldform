...
function newChart(chartID, startDate, graphData, color) {
    ...
    tooltip: {
        formatter: function() {
                return ''+
                Highcharts.dateFormat('%H:00', this.x) +': <strong>'+ this.y +'</strong>Â°' + '<br><span style="font-size: 7px; text-transform: uppercase; opacity: 0.8">' +
	...
	$.each(data, function(i, obj) {
		...
		$.each(obj.day.entry, function(x, day) {
			...
			if(i == 1) {
				...
				$('.slide:eq('+index+') .weather').append('<div class="info">' + city + ' <span>' + temp + 'Â°</span>'  + '</div>');
...
//		}
//		
//		$('.slide:eq('+index+') .forecast').append('<div class="day"><div class="icon ' + con.replace(/ /g, '_').toLowerCase() + '"></div><div class="info">' + day + ' <span class="temp" data-temp="'+high+','+low+'">' + high + 'Â° / ' + low + 'Â°</span><span class="con">'+con+'</span></div></div>');
//		
//		if(it==0) {
//			$('.slide:eq('+index+') .forecast .day:nth-child(1) .info .con').append('<span data-wind="'+o_wind+'"> â€” ' + wind + ' ' + w_unit + '</span>');
//		}
//	});
}
	...
	$.each(sounds, function(i) {
		...
		request.onload = function () {
			context[name].decodeAudioData(request.response, function (buffer_) {
				//console.log('[Sun] â€” Success: Decoded audio file: ' + sounds[i]);
				buffer[name] = buffer_;
			}, function() {
				console.log('[Sun] â€” Error: Couldn\'t decode audio file: ' + sounds[i]);
			});
		};
		request.send();
	});
...
$(document).ready(function() {
	if (!window.navigator.standalone) {
		...
		if(!window.navigator.onLine) {
			...
			// #First Time
			// _If a user visits for the first time â€” fill in local storage values_
			...
			// #Declaration
			// _This will fuck completely, if a browser doesn't support localStorage â€” but this is an iOS concept_ //
			var cities = [
			...
			$('#settings ul li').live('tap', function() {		
				...
				if($(this).parent().attr('class') == 'temperature' && $(this).attr('class') != 'current') {
					...
					$('.slide > .weather .info > span, .forecast .data strong, ul.underlay li .temp strong').each(function() {
						$(this).html(convert(parseInt($(this).html().split('Â°')[0]), unit) + 'Â°');
					});
